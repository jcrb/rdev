...
![plot of chunk semaine](figure/semaine1.png) 
...
```r
avc <- dpr[substr(dpr, 1, 3) >= "I60" & substr(dpr, 1, 3) < "I65" | substr(dpr, 
    1, 3) == "G46"]
navc <- unique(avc)
a <- as.character(avc)
a <- gsub(".", "", a, fixed = TRUE)
tab1(a, horiz = TRUE, sort.group = "decreasing", main = "AVC aux urgences (hors filière UNV", 
    missing = FALSE)
```
...
## I604            1      0.0      0.1
## I621            1      0.0      0.1
##   Total     34741    100.0    100.0
```

```r

avc_ait <- dpr[substr(dpr, 1, 3) >= "I60" & substr(dpr, 1, 3) < "I65" | substr(dpr, 
    1, 3) == "G46" | substr(dpr, 1, 3) == "G45"]
```

###on crée un dataframe avec 10 colonnes:

```r
CIM10 <- d1[as.character(d1$DP) != "NULL", c("DP", "CODE_POSTAL", "ENTREE", 
    "FINESS", "GRAVITE", "ORIENTATION", "MODE_SORTIE", "AGE", "SEXE", "TRANSPORT")]
```
...
AVC <- CIM10[substr(CIM10$DP, 1, 3) >= "I60" & substr(CIM10$DP, 1, 3) < "I65" | 
    substr(CIM10$DP, 1, 3) == "G46" | substr(CIM10$DP, 1, 3) == "G45", ]
```
...
![plot of chunk unnamed-chunk-63](figure/unnamed-chunk-631.png) 

```r
tab1(h, missing = FALSE)
```

![plot of chunk unnamed-chunk-63](figure/unnamed-chunk-632.png) 
...
## w
##   Sun   Mon  Tues   Wed Thurs   Fri   Sat 
## 12.70 15.62 14.12 11.63 15.48 16.90 13.55
```
...
```r
ait <- CIM10$DP[substr(CIM10$DP, 1, 3) == "G45" & substr(CIM10$DP, 1, 4) != 
    "G454"]
